      ...
      <div className="px-6 pt-8 pb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center"
            ...
            <PiggyBank size={18} skin={pigSkin} />
          </div>
      ...
      <div className="px-6 pb-6 flex flex-col items-center"
        ...
        <div className="grid grid-cols-3 gap-2 w-full mt-3">
          ...
          <div className="p-2 rounded-xl text-center"
            ...
            <div className="text-cyan-400" style={{ fontWeight: 800, fontSize: 15 }}>{totalFeedCount}</div>
            <div className="text-white/30" style={{ fontSize: 9 }}>Feeds</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-5" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.07)' }}>
          <motion.div
            ...
            style={{ background: 'linear-gradient(90deg, #F97316, #FBBF24)', width: `${levelProgressPercent}%` }}
            initial={{ width: '0%' }}
            animate={{ width: `${levelProgressPercent}%` }}
            transition={{ duration: 1.2, ease: 'easeOut' }}
          />
        </div>
        <div className="mt-2 text-white/20" style={{ fontSize: 10 }}>{xpToNextLevel.toLocaleString()} XP to Level {level + 1}</div>
      </div>
    </aside>
  );
};
